...
HRESULT ClrProcess::EvaluateExpression(CLRDATA_ADDRESS rootAddr, LPCWSTR expression, CLRDATA_ADDRESS *result)
{
	...
	for (int a = 0; a < len; a++)
	{
		if (str[a] == L'.')
		{
			...
			hr = GetFieldValuePtr(currAddr, buffer.data(), &currAddr);
			RESET_BUFFER
		}
		else if (str[a] == L'[')
		{
